...
def make_two_col(prs, s, idx, total):
    ...
    for ci, items in enumerate([col1, col2]):
        for i, item in enumerate(items[:5]):
            ...
            _oval(slide, x + icon_circle//2, y + I(0.12) + icon_circle//2, icon_circle//2, ac)
            ic = _pick_icon(item)
            _add_icon(slide, x + (icon_circle - icon_sz)//2, y + I(0.12) + (icon_circle - icon_sz)//2, icon_sz, WHITE, icon_name=ic)
            _tb(slide, item, x + I(0.55), y + I(0.06), col_w - I(0.85), I(0.70),
                sz=11, color=DARK, font="Calibri")
...
LAYOUT_MAP = {
    ...
    "donut": make_donut,
    "two-col": make_two_col, "two_col": make_two_col, "timeline": make_timeline, "pillars": make_pillars,
