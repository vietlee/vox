class Admin::FeedbackBoardsController < Admin::BaseController
  ...
  def export
    ExportFeedbackJob.perform_later(@board.id, current_user.id)
    redirect_to feedback_board_feedbacks_path(@board), notice: t("feedback_boards.export_queued")
  end

  def ai_summarize
    require_ai_feature!(:ai_analysis)
    require_credits!(3)
    job = AiJob.create!(workspace: current_workspace, user: current_user, job_type: "feedback_analysis", resource_type: "FeedbackBoard", resource_id: @board.id, credits_cost: 3)
