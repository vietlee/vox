class Api::Learner::V1::FlashcardAssignmentsController < Api::Learner::V1::BaseController
  ...
  def study
    if @assignment.completed?
      @assignment.update_columns(
        cards_reviewed: 0,
        status: FlashcardAssignment.statuses[:in_progress],
        completed_at: nil
      )
    else
  ...
  def review
    ...
    render json: {
      ...
      progress: @assignment.cards_reviewed,
      total: total,
  ...
  def assignment_json(a)
    { token: a.token, status: a.status, progress: a.cards_reviewed,
      progress_pct: a.progress_pct, cards_reviewed: a.cards_reviewed,
      total_cards: a.flashcard_deck.flashcards.count }
  end
